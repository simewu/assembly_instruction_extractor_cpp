...
if __name__ == '__main__':
	...
	assert baselineRow[0] == 'gdb_baseline.cpp', 'The initial baseline row was not found'
	baselineJSON = json.loads(baselineRow[5].replace("'", '"'))
	for row in reader:
		fileName = row[0]
		...
		addressStart = row[3]
		addressEnd = row[4]
		opcodes = json.loads(row[5].replace("'", '"'))
		...
		for op in opcodes:
			...
			try:
				...
				if category is None:
					...
					elif op in ['cltd', 'cltq']: category = 'CONVERT'
					elif op in ['callq']: category = 'CALL'
					elif op in ['pushq']: category = 'PUSH'
					elif op in ['shrl']: category = 'SHIFT'
					elif op in ['retq']: category = 'RET'
					else:
						print(f'No category for " {op} " ({opcodes[op]})! Please add it to postProcess_categorize.py line 85.')
						sys.exit()
			...
			if category in ['BITBYTE', 'BROADCAST', 'CMOV', 'CONVERT', 'DATAXFER', 'POP', 'PUSH', 'SETCC', 'XSAVE']:
				...
				print('No category for', op, f'({opcodes[op]})', '->', category, 'Please add it to postProcess_categorize.py line 103.')
				sys.exit()
		...
		line += '"' + str(opcodes) + '",'
		if fileName not in experimentData: experimentData[fileName] = ''
		experimentData[fileName] += line + '\n'
		print('   ', categorizedOutputs)

	for key in experimentData:
		outputFile.write(experimentData[key])
